  ...
  describe('When logged in', function () {
    ...
    describe('When there are Blogs', function () {
      ...
      it('Blogs should be sorted by likes', function () {
        cy.get('.blogListItem').eq(0).should('contain', 'API Title')
        cy.get('.blogListItem').eq(1).should('contain', 'API Title 2')
        cy.contains('API Title').click()
        cy.contains('API Title').parent().contains('Like').click()
        cy.visit('http://localhost:3000')
        cy.contains('API Title 2').click()
        ...
        cy.wait(500)
        cy.visit('http://localhost:3000')
        cy.get('.blogListItem').eq(0).should('contain', 'API Title 2')
        cy.get('.blogListItem').eq(1).should('contain', 'API Title')
      })
    })
  })
})
